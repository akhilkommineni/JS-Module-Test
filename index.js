...
rulesHide.addEventListener('click', () => {
  rulesContainer.classList.toggle("hide-container");
  middleContainer.classList.toggle("homepagecontainer-main-toggle");
});
userChoices.forEach((eachEle) => {
  eachEle.addEventListener('click', () => {
    const userChoice = eachEle.getAttribute('value');
    ...
    if (winner === 'player') {
      ...
      playerWonElement.classList.toggle('hide-container', false); 
      playerWonElement.querySelector('.tryagain').addEventListener('click', () => {
          reseteverything()
        });
      playerWonElement.querySelector("#player-icon").src = icons[userOption];
      playerWonElement.querySelector("#comp-icon").src = icons[computerOption];
      ...
      TieElement.classList.toggle('hide-container', true);
      playerWonElement.classList.toggle('hide-container', true);
      computerWonElement.querySelector('.tryagain').addEventListener('click', () => {
          reseteverything();
        }); 
      computerWonElement.querySelector("#player-icon").src = icons[userOption];
      computerWonElement.querySelector("#comp-icon").src = icons[computerOption];
    } else if (winner === 'draw') {
      computerScore++;
      computerScoreEl.textContent = computerScore;
      playerScore++;
      ...
      playerWonElement.classList.toggle('hide-container', true);
      TieElement.querySelector('.tryagain').addEventListener('click', () => {
          reseteverything()
        });
      TieElement.querySelector("#player-icon").src = icons[userOption]; 
      TieElement.querySelector("#comp-icon").src = icons[computerOption];
    }
  })
}) 
...
function reseteverything() {
  homePageElement.classList.remove('hide-container');
  playerWonElement.classList.contains('hide-container') ? '' : playerWonElement.classList.add('hide-container');
